...
class PositionHistoricalPriceResolverServiceTest {
    ...
    @Test
    void resolveViopFallsBackToHistoryWhenPriceAtMissing() {
        LocalDate d = LocalDate.of(2026, 4, 29);
        when(marketDataClient.getViopPriceAt(any(), eq(d))).thenReturn(Optional.empty());
        Instant t = d.atTime(10, 0).atZone(ZoneId.of("Europe/Istanbul")).toInstant();
        when(marketDataClient.findViopPriceOnCalendarDay(eq("USDTRY1226"), eq(d)))
                .thenReturn(Optional.of(new ViopPriceAtRow(
                        "USDTRY1226",
                        d.atStartOfDay(),
                        d.atTime(10, 0),
                        new BigDecimal("55.94"),
                        "EXACT",
                        "VIOP_HISTORY",
                        "OK")));
        ...
        PositionHistoricalPriceResolveDto r = service.resolveViop("USDTRY1226", d);
        assertThat(r.getMatchType()).isEqualTo(HistoricalPriceMatchType.EXACT);
        assertThat(r.getPrice()).isEqualByComparingTo("55.94");
    }

...
class GlobalExceptionHandlerTest {
    ...
    @Test
    void notFoundShouldIncludeResourceCode() {
        HttpServletRequest request = request("/api/resource");

        ResponseEntity<ApiResponse<?>> response = handler.handleNotFound(new IllegalStateException("not found"), request);

        assertThat(response.getStatusCode().value()).isEqualTo(404);
        Map<String, Object> errors = castErrors(response.getBody());
        assertThat(errors.get("code")).isEqualTo("RESOURCE_NOT_FOUND");

...
/**
 * Keycloak realm güvenlik ayarları ve brute-force yönetimi.
 */
@Service
...
public class KeycloakRealmSecurityClient {
    ...
    /**
     * Realm oturum ve token ömür ayarlarını Keycloak'tan okur.
     */
    ...
    /**
     * Realm güvenlik/oturum süresi alanlarını günceller.
     */
    ...
    /**
     * Belirtilen realm rolündeki kullanıcıları sayfalı listeler.
     */
    ...
    /**
     * Kullanıcının Keycloak requiredActions listesini döner.
     */
    ...
    /**
     * Eksikse kullanıcıya required action ekler.
     */
    ...
    /**
     * Kullanıcının tüm required action'larını temizler.
     */
    public void clearAllRequiredActions(String userId) {
        String json = doGet("/admin/realms/" + adminProperties.getRealm() + "/users/" + userId);
        try {
            ObjectNode node = (ObjectNode) objectMapper.readTree(json);
            node.set("requiredActions", objectMapper.createArrayNode());
            doPut("/admin/realms/" + adminProperties.getRealm() + "/users/" + userId, node.toString());
        } catch (Exception e) {
            throw new IllegalStateException("Keycloak requiredActions temizlenemedi", e);
        }
    }

    /**
     * Varsa belirtilen required action'ı kaldırır.
     */
    ...
    /**
     * Kullanıcının OTP/TOTP credential'ı olup olmadığını kontrol eder.
     */
    ...
    /**
     * Realm required action varsayılanını enabled yapar.
     */
    public boolean ensureRequiredActionDefaultEnabled(String alias) {
        return setRequiredActionDefault(alias, true);
    }

    /**
     * Realm required action varsayılanını disabled yapar.
     */
    public boolean ensureRequiredActionDefaultDisabled(String alias) {
        return setRequiredActionDefault(alias, false);
    }

    private boolean setRequiredActionDefault(String alias, boolean defaultAction) {
        ...
        try {
            ...
            for (JsonNode node : root) {
                ...
                boolean currentDefault = actionNode.path("defaultAction").asBoolean(false);
                if (currentDefault == defaultAction) {
                    return false;
                }
                ObjectNode update = actionNode.deepCopy();
                update.put("enabled", true);
                update.put("defaultAction", defaultAction);

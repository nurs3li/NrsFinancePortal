package com.nurseli.nrsfinanceportal.infrastructure.logging;
...
/**
 * Log4j2 Kafka Appender; log event'lerini JSON olarak application-logs topic'ine yazar.
 */
...
public class KafkaLogAppender extends AbstractAppender {
    ...
    @Override
    /**
 * Appender başlatılırken Kafka producer'ı hazırlar.
 */
    public void start() {
        super.start();
    ...
    @Override
    /**
 * Appender kapanırken Kafka producer'ı kapatır.
 */
    public void stop() {
        if (producer != null) {
            producer.close();
    ...
    @Override
    /**
 * LogEvent'i Kafka log topic'ine JSON olarak yayımlar.
 */
    ...
    private String formatTimestamp(org.apache.logging.log4j.core.time.Instant instant) {
        ...
        try {
            // Log4j2 Instant: getEpochSecond() + getNanoOfSecond() veya getEpochMillisecond()
            long epochMillis;
            try {
                epochMillis = instant.getEpochMillisecond();
            } catch (NoSuchMethodError e) {
                // Eski API: epoch second + nano

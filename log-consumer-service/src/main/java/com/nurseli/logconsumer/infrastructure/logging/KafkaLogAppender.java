package com.nurseli.logconsumer.infrastructure.logging;
...
/**
 * Log4j2 appender; uygulama loglarını JSON olarak Kafka topic'ine yazar.
 * {@code correlationId}, trace ve audit alanları MDC bağlamından taşınır.
 */
...
public class KafkaLogAppender extends AbstractAppender {
    ...
    /** {@code start} — Kafka producer'ı başlatır. */
    @Override
    public void start() {
        super.start();
    ...
    /** {@code stop} — Kafka producer'ı kapatır. */
    @Override
    public void stop() {
        if (producer != null) {
            producer.close();
    ...
    /**
     * {@code append} — Log event'ini JSON belgeye dönüştürüp Kafka topic'ine gönderir;
     * gönderim hatası olursa sessizce yutulur.
     */
    @Override
    ...
    /** {@code putAuditFields} — MDC'deki audit alanlarını JSON belgeye ekler. */
    ...
    /** {@code putTraceIds} — MDC'deki trace/span kimliklerini JSON belgeye ekler. */
    ...
    /** {@code firstCtx} — Verilen anahtar adaylarından ilk dolu MDC değerini döner. */
    ...
    /** {@code formatTimestamp} — Log4j zaman damgasını ISO-8601 string'e çevirir. */
    ...
    /** {@code createAppender} — Log4j2 plugin fabrikası; yapılandırmadan appender örneği oluşturur. */
    @PluginFactory

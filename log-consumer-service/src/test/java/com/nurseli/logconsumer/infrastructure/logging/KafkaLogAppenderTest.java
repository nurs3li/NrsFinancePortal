...
class KafkaLogAppenderTest {
  ...
  void append_withoutStartedProducer_isNoOp() {
    KafkaLogAppender appender =
        KafkaLogAppender.createAppender("test", null, "application-logs", "localhost:9092", "log-consumer-service");
  ...
  void append_sendsJsonWithCorrelationAndServiceName() throws Exception {
    KafkaLogAppender appender =
        KafkaLogAppender.createAppender("test", null, "logs-topic", "localhost:9092", "log-consumer-service");
    @SuppressWarnings("unchecked")
    KafkaProducer<String, String> producer = mock(KafkaProducer.class);
    injectProducer(appender, producer);

    appender.append(sampleEvent("indexed"));
    ...
    assertThat(json).contains("\"serviceName\":\"log-consumer-service\"");
    assertThat(json).contains("\"message\":\"indexed\"");
  ...
  private static LogEvent sampleEvent(String message) {
    ...
    when(ctx.containsKey(eq("userId"))).thenReturn(false);

"""Generate weekly seed history for curated Turkey USD eurobonds (broker mid proxies)."""
import json
import math
...
BONDS = [
    ...
    ("US900123AY60", 6.88, 99.59, 6.90),
    ("US900123DA57", 5.95, 98.34, 6.28),
    ("US900123BB58", 7.25, 104.15, 6.71),
    ("US900123BJ84", 6.00, 85.95, 7.53),
    ("US900123CG37", 6.63, 87.97, 7.82),
    ("US900123CM05", 5.75, 77.51, 7.92),
]

OUT = Path(__file__).resolve().parents[1] / "src/main/resources/eurobond-seeds"
...
def build_points(
    ...
    # Per-ISIN path: same terminal targets, different start level, phase and micro-volatility.
...
def main():
    ...
    for isin, coupon, price, yld in BONDS:
        payload = {
            "isin": isin,
            "source": "SEED_WEEKLY_PROXY",
            "note": "Placeholder weekly history until EVDS per-ISIN or bank feed is wired.",
